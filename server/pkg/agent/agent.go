...
// Gemini, Pi, Cursor, Kimi, Kiro, Nanobot). It mirrors the happy-cli
...
func New(agentType string, cfg Config) (Backend, error) {
	...
	case "nanobot":
		return &nanobotBackend{cfg: cfg}, nil
	default:
		return nil, fmt.Errorf("unknown agent type: %q (supported: claude, codex, copilot, opencode, openclaw, hermes, gemini, pi, cursor, kimi, kiro, nanobot)", agentType)
	}
}
...
var launchHeaders = map[string]string{
	...
	"kimi":     "kimi acp",
	"kiro":     "kiro-cli acp",
	"nanobot":  "nanobot gateway (websocket)",
}
